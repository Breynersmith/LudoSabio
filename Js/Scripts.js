...
let estado = {
    ...
    preguntasGuardadas: [],
    stateMessageSuccess: false,
    stateMessageError: false,
    stateModalOperation: false,
    stateModalWin: false
};
...
const el = {
    ...
    modalWinVidas: document.getElementById("modalWinVidas"),
};

function sincronizarState() {

    estado.stateModalOperation === false ? el.overlayOperacion.classList.add("hidden") : el.overlayOperacion.classList.remove("hidden");
    estado.stateMessageSuccess === false ? el.successMessage.classList.add("hidden") : el.successMessage.classList.remove("hidden");
    estado.stateMessageError === false ? el.errorMessage.classList.add("hidden") : el.errorMessage.classList.remove("hidden");
    estado.stateModalWin === false ? el.modalWinVidas.classList.add("hidden") : el.modalWinVidas.classList.remove("hidden");
...
function loopGusanito() {
    ...
    if (head.x === gusanito.food.x && head.y === gusanito.food.y) {
        estado.stateModalOperation = true;
        estado.stateModalOperation == true ? gusanito.awaitingAnswer = true : gusanito.awaitingAnswer = false;
...
function modalEnviar() {
    ...
    if (respuesta === gusanito.currentResult) {
        ...
        gusanito.streak++;
        document.getElementById("aciertosG").innerText = gusanito.aciertos;
        estado.stateMessageSuccess = true;
        estado.stateMessageError = false;
        sincronizarState();

        if (gusanito.streak >= 3) {
            if (gusanito.loopId) clearInterval(gusanito.loopId);
            document.removeEventListener("keydown", teclaGusanito);
            estado.stateModalWin = true;
            sincronizarState();

            setTimeout(() => {
                estado.stateMessageSuccess = false;
            }, 500);
            sincronizarState();
            estado.vidas += 2;
            actualizarVidasUI();
...
function modalCerrar() {
    estado.stateModalOperation = false;
    gusanito.awaitingAnswer = true;
    estado.stateMessageError = false;
    estado.stateMessageSuccess = false;
